    ...
    while not frontier.empty():
        ...
        for movie_id, neighbor_id in neighbors_for_person(node.state):
            if neighbor_id not in explored and not frontier.contains_state(neighbor_id):
                # If we find the target, reconstruct the path
                if neighbor_id == target:
                    path = []
                    path.append((movie_id, neighbor_id))  # Add the final connection
                    # Follow the parent nodes to build the full path
                    while node.parent is not None:
                        path.append((node.action[0], node.state))
                        node = node.parent
                    path.reverse()  # Reverse the path to start from the source
                    return path

                # Otherwise, add the node to the frontier
                child_node = Node(state=neighbor_id, parent=node, action=(movie_id, neighbor_id))
